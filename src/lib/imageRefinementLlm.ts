...
export async function generateImageRefinementPlanWithLlm(input: {
  ...
  const watermarkRequirement = input.removeWatermarks
    ? `
- 用户明确要求去除叠加水印和账号角标。只处理明显后期叠加在画面上的平台水印、账号角标或来源标记；如存在，editPrompt 必须明确要求去除，并根据邻近画面自然补全背景，不留下模糊块、涂抹痕迹、重复纹理或明显修补边界。
    ...
    const resolvedPlan = input.removeWatermarks
      ? {
          ...
          globalEditRules: [
            ...
            "仅去除明显后期叠加的平台水印、账号角标或来源标记；自然补全背景，不得留下模糊块、涂抹痕迹、重复纹理或明显修补边界。物体本身固有的文字、Logo、刻印、包装信息、餐具标识、衣物图案、店内招牌、菜单文字和其他真实场景元素必须保留。"
          ],
          images: plan.images.map((image) => ({
            ...image,
            editPrompt: `${image.editPrompt}\n仅检查并去除明显后期叠加的平台水印、账号角标或来源标记；如存在则自然补全背景，不留下模糊块、涂抹痕迹、重复纹理或明显修补边界。原始物体本身固有的文字、Logo、刻印、包装信息、餐具标识、衣物图案、店内招牌、菜单文字和其他真实场景元素必须保留，不得去除、遮盖或重绘。`,
            reviewNotes: `${image.reviewNotes}；仅确认平台水印、账号角标和悬浮来源标记已去除且补图自然。物体本身固有的文字、Logo、刻印和真实场景文字允许保留，不得因此重试或判定失败。`
          }))
        }
      : plan;
...
export async function generateMixedImagePlanWithLlm(input: {
  ...
  const watermarkRequirement = input.removeWatermarks
    ? "\n- 用户开启去水印：所有真实素材的 editPrompt 只需去除明显后期叠加的平台水印、账号角标或来源标记，并自然补全背景；AI 辅助图不需要写去水印要求。不得去除、遮盖、重绘或作为验收失败项处理物体本身固有的文字、Logo、刻印、包装信息、餐具标识、衣物图案、店内招牌、菜单文字或其他真实场景元素。"
    : "";
