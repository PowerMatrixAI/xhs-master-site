...
JSON 格式：
{
  ...
  let response = await completeWithBackendAi({
    ...
    ...(hasKnowledgeSources ? { knowledgeSnapshotId: input.knowledgeSnapshotId, knowledgeSourceKeys: input.knowledgeSourceKeys } : {}),
    input: promptInput
  });

  // 故事视频等新建任务的知识来源可能尚未形成可引用 key；空文本时仅重试一次，去掉知识库注入。
  if (!response.ok && /(?:empty text|空文本)/iu.test(response.error || "")) {
    response = await completeWithBackendAi({
      instructions: "你是擅长模仿小红书爆款表达的中文文案作者。严格输出合法 JSON，不输出解释、Markdown 代码块或版本生成过程。",
      input: promptInput
    });
  }
