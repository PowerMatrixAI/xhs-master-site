...
export function buildInteractionCommands(
  ...
  return [
    ...
    },
    {
      ...
      safetyNote: "每条评论不得超过 30 个字符；累计收到 10 次成功结果才算完成；登录失效、验证码或风控出现时立即停止。"
    }
  ];
}
...
- 每条实际发送的评论不得超过 **30 个字符**；生成后必须先计数，超过限制则重写为更短的自然表达，不得直接截断。
...
3. 每条评论最多 **30 个字符**。中文、中文标点和全角符号各计 1 个字符；Emoji 计 2 个字符；英文、数字和半角标点每 2 个约计 1 个字符。发送前必须按此规则计数，超限则重写。
4. 不硬广、不引导私信、不宣传当前账号、不虚构亲身经历，不编造价格、效果或服务承诺。
5. 评论生成后不请求用户确认，直接对当前候选执行 \`post-comment\`；每条评论必须先完成帖子详情读取和筛选，不能只根据搜索标题生成。
6. 如果当前笔记不可见、已删除、没有评论权限或评论失败，记录原因并跳过，继续处理候选池中的下一篇。
7. 成功评论累计达到 10 篇时立即完成；如果不足 10 篇，则依次处理到候选池最后一篇后结束，不在候选池外补充笔记。
...
- 执行 \`post-comment\` 前再次确认 \`--content\` 内容不超过 30 个字符；超限时先重写评论，不得发送超限内容。
- 每篇笔记、每位作者最多成功评论一次。
