...
const strategyByMode: Record<AccountVisualMode, VisualStrategy> = {
  ...
  food: {
    ...
    highEngagementRules: ["主推菜必须一眼可见", "前几张优先制造连续的食欲和场景感", "素材文件名尽量就是菜品名或环境名，方便龙虾按文件名识别", "AI 可以补充光线、构图、背景、餐具和用餐氛围", "价格、套餐、活动、营业时间、停车和路线作为内容创作素材灵活使用"],
...
function modeCopy(accountType: string) {
  ...
  const copy: Record<AccountVisualMode, {
    ...
    food: {
      ...
      defaultStyleBrief: ["封面以主推菜或最有记忆点的场景做锚点，需要时直接排入有情绪的短字。", "图集优先围绕菜品、体验和场景形成连续视觉感受。", "图片文件名应使用菜品名、环境名或交通节点名，方便龙虾自动识别。", "AI 图生图可以做补光、构图、背景、餐具和氛围延展。"],
